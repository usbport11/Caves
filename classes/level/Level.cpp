...
bool MLevel::ConvertToTiles() {
	...
	const char TT_NONE = 0;
	const char TT_FLOOR = 1;
	const char TT_WALL_FULL = 2;
	const char TT_WALL_PART = 3;
...
void MLevel::Inverse() {
	...
	for(int i = 0; i<TilesCount[0]; i++) {
		for(int j = 0; j<TilesCount[1]; j++) {
			Map[i][j] = (char)!(bool)Map[i][j];
		}
	}
}

void MLevel::ReplaceAll(char From, char To) {
	if(!Map) return;
	for(int i = 0; i<TilesCount[0]; i++) {
		for(int j = 0; j<TilesCount[1]; j++) {
